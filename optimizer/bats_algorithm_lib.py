...
class BatAlgorithm():
    def __init__(self, D, NP, N_Gen, A, r, Qmin, Qmax, Lower, Upper, function):
        ...
        self.f_max = float('inf')  #minimum fitness
    ...
    def init_bat(self):
        ...
        self.Fitness = self.Fun(self.D, self.Sol)
        self.best_bat()
    ...
    def move_bat(self):
        ...
        for t in range(self.N_Gen):
            ...
            Fnew = self.Fun(self.D, S)

            for i in range(self.NP):
                rnd = np.random.random_sample()

                if (Fnew[i] >= self.Fitness[i]) and (rnd < self.A):
                    for j in range(self.D):
                        self.Sol[i][j] = S[i][j]
                    self.Fitness[i] = Fnew[i]

                if Fnew[i] >= self.f_max:
                    for j in range(self.D):
                        self.best[j] = S[i][j]
                    self.f_max = Fnew[i]
